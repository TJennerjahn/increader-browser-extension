...
3. Enter an email and password for Increader Cloud, or choose Continue with
   ...
   closes when the callback returns to the Extension. Chrome uses its managed
   Web Auth Flow; Firefox uses a popup scoped to the single Mozilla callback
   origin because its managed flow rejects Clerk's intermediate callback. The
   cog opens the separate instance screen for selecting a self-hosted origin
   and using that instance's normal credentials; Google is then hidden.
